...
- accessing an input using useRef()
- validate the error state 
-  error state Vs controlled inputs using the .value prop.  In this example  it sets to text to lowercase (a specific value) even if the user has caps lock on.  What implications, does this have in relation to mobile and touch screens - isn't it better to have the user be aware of the implications of what they are doing with an error message, rather than just changing it in the code?
