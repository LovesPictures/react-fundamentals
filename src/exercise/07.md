...
- Adding a unique key prop to the `<li>` (children) allows React to track the element from the default state over time, without key={item.id} - its not clear to React which items have been updated.  DO NOT USE THE INDEX OF THE ITEM, use a unique identifier e.g.ID- That's the default behaviour.

- useful for anything that maintains state over time e.g. an input
